...
var form_action = function(this_item) {
  var check_form = function(){
    ...
    if (!item_id) {
      show_msg("无法获取商品id", true);
      return false;
    }
    if (!title) {
      show_msg("无法获取商品标题", true);
      return false;
    }
    if (!image) {
      show_msg("无法获取商品图片", true);
      return false;
    }
    if (!price) {
      show_msg("无法获取商品价格", true);
      return false;
    }
    return true;
  }
...
var item_ready = function(this_item) {
  $(".dgsc-pc>i").click(function(){
    ...
    $(".order-buy .order-info .order-list-info .list-cont .godds-specification").text(sku_txt);
    ...
    if (sku_id) {
      ...
      $(".order-buy .order-info .order-list-info .list-price-nums p").text("￥" + price);
      $(".order-buy .order-info .subtotal .price").text("￥" + total);
      $("input#form-sku-id").val(sku_id);
  ...
  $(".order-buy .order-info .buy-nums .input-nums").change(function(){
    ...
    var quantity = this_item.get_quantity(sku_id);
    if (!num || num < 1) {
      show_msg("数量请输入正整数", true);
      $(this).val(1);
      num = 1;
    } else if (num > quantity) {
      show_msg("此商品最多可购买" + quantity + "件", true);
      $(this).val(quantity);
      num = quantity;
    }
    if (num == 1) {
...
var tb_item = function(mkt, item_id) {
  ...
  var load_item = function(this_item){
    ...
    bgp_call("get_json", url, function(response) {
      ...
      $(".order-buy .order-info .order-list-info .list-price-nums p").text("￥" + item_info.priceUnits[0].price);
...
var unsupport_item = function(mkt, item_id) {
  var load_item = function(this_item) {
    ...
    if (msg[mkt]) {
      show_msg(msg[mkt], false);
    } else {
      show_msg("暂不支持该商品", false);
    }
  }
  var get_shipfee = function() {
    return -1;
